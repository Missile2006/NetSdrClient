...
namespace InfrastructureTests
{
    ...
    public class ArchitectureTests
    {
        ...
        public void UI_Should_Not_Depend_On_Infrastructure()
        {
            ...
            var failing = result.FailingTypeNames ?? new string[0];

            Assert.That(result.IsSuccessful,
                $"UI шар не повинен залежати від Infrastructure. Порушення: {(failing.Count == 0 ? "немає" : string.Join(", ", failing))}");
        }
        ...
        public void Infrastructure_Should_Not_Depend_On_UI()
        {
            ...
            Assert.That(result.IsSuccessful,
                $"Infrastructure не повинен залежати від UI. Порушення: {(failing.Count == 0 ? "немає" : string.Join(", ", failing))}");
        }
    }
}
